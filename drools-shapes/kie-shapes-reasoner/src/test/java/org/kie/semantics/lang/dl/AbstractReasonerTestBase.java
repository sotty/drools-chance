...
import org.kie.semantics.builder.DLReasonerBuilder;
import org.kie.semantics.builder.DLReasonerBuilderImpl;
...
public class AbstractReasonerTestBase {
	...
	public OWLOntology init( String dlFile ) {
		...
		OWLOntology ontologyDescr = factory.parseOntology( AbstractReasonerTestBase.class.getResourceAsStream( dlFile ) );
		currentOntology = ontologyDescr;
		currentSource = dlFile;

		String tableau = factory.buildTableauRules( ontologyDescr,
		                                            new String[] { "/FALC_TableauBuilderVisitor.drl", "/FALC_CommonVisitor.drl" } );
		// System.err.println("<<<" + tableau + ">>>");
		assertFalse( StringUtils.isEmpty( tableau ) );


		KieHelper helper = new KieHelper();
		tableauKB = helper.addContent( tableau, ResourceType.DRL )
		                  .addFromClassPath( "/fuzzyDL/Main.drl" )
