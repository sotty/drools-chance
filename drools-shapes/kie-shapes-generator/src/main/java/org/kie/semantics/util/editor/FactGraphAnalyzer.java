package org.kie.semantics.util.editor;
...
public class FactGraphAnalyzer  {
    ...
    private static Map<Class, Map<String,RelationDescriptor>> relCache = new HashMap<>();


    public Graph analyzeObject( Thing thing ) {

        Map<Object, Node> cache = new IdentityHashMap<>();
            ...
            if ( val != null ) {
                ...
                for ( Object x : list ) {
                    if ( isDataType( x.getClass() ) ) {
                        addDataRelation( root, key, x, m, graph );
                    } else {
                        exploreObjectRelation( root, key, x, m, graph, cache );
                    }
                }
            }
        }
    ...
    private void exploreObjectRelation(Node root, String key, Object val, Method getter, Graph graph, Map<Object, Node> cache) {
    ...
    public static  Map<String, RelationDescriptor> extractRelations( Object val ) {
        if ( ! relCache.containsKey( val.getClass() ) ) {
            Map<String,RelationDescriptor> props = new HashMap<>();
    ...
    private void addDataRelation( Node root, String rel, Object val, Method getter, Graph graph ) {
