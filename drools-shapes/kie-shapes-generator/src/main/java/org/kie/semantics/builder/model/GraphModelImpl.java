...
public class GraphModelImpl extends ModelImpl implements GraphModel {
    ...
    private Graph<Concept, Relation> cgraph = new SparseMultigraph<>();
    ...
    public void display() {
        ...
        KKLayout<Concept, Relation> layout = new KKLayout<>( cgraph );
        ...
        VisualizationViewer<Concept, Relation> vv = new VisualizationViewer<>( layout );
        ...
        Transformer<Relation, Stroke> edgeStrokeTransformer =
		        rel -> rel.getProperty().contains("subConceptOf") ? sccStroke : edgeStroke;

        Transformer<Concept,Paint> vertexPaint = c -> c.getIri().contains( "XMLSchema" )  ? Color.GREEN : Color.BLUE;
...
//                cgraph.get, cgraph.edgeFactory);
    ...
    public void addRelationEdge( Relation rel ) {
        ...
        if ( obj == null &&  rel instanceof PropertyRelation ) {
            // literal aka datatype, most probably
            Concept k = new Concept( IRI.create( rel.getObject() ),
                                     null,
                                     ((PropertyRelation) rel).getTarget().getName(),
                                     true );
            cgraph.addVertex( k );
            obj = k;
        }
        if ( sub != null && obj != null ) {
            ...
            System.err.println(" WARNING : Broken edge for " + rel.getProperty() + " | obj = " + obj );
        }
